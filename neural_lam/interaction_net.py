...
class InteractionNetOld(pyg.nn.MessagePassing):
    """
    ...
    def aggregate(self, inputs, index, ptr, dim_size):
        ...
        * only aggregate to number of receiver nodes.
        ...
        # print(aggr.shape, inputs.shape)
        # assert False
        # torch.Size([1, 729, 128]) torch.Size([1, 12716, 128])
        return aggr, inputs
...
#     def forward(self, node_rep, edge_rep, edge_index):
#         """
#         Apply interaction network to update the representations of receiver
#         nodes, and optionally the edge representations.
...
#         # # Store number of receiver nodes according to edge_index
#         # self.num_rec = edge_index[1].max() + 1
#         # edge_index[0] = edge_index[0] + self.num_rec  # Make sender indices
#         # after rec

#         # Always concatenate to [rec_nodes, send_nodes] for propagation,
#         # but only aggregate to rec_nodes
#         # node_rep = torch.cat((rec_rep, send_rep), dim=-2)
#         edge_rep_aggr, edge_diff = self.propagate(
#             edge_index, x=node_rep, edge_attr=edge_rep
#         )
#         rec_diff = self.aggr_mlp(torch.cat((rec_rep, edge_rep_aggr), dim=-1))
#         print(rec_rep.shape, edge_rep_aggr.shape, edge_rep.shape)
#         # Residual connections
#         rec_rep = rec_rep + rec_diff
...
class InteractionNet(pyg.nn.MessagePassing):
    def __init__(
        ...
        edge_mlp_recipe = [3 * input_dim] + [hidden_dim] * (hidden_layers + 1)
        node_mlp_recipe = [2 * input_dim] + [hidden_dim] * (hidden_layers + 1)

        self.NodeMLP = utils.make_mlp(node_mlp_recipe)
        self.EdgeMLP = utils.make_mlp(edge_mlp_recipe)
    ...
    def forward(self, sender, receiver, edge_feature):
        node_reps = torch.cat((sender, receiver), dim=-2)
        edge_out, aggr = self.propagate(
            self.edge_index, x=node_reps, edge_feature=edge_feature
        )
        x = receiver

        # edge_out, aggr = self.propagate(
        # edge_index, x=(x, x), edge_feature=edge_feature)

        node_out = self.NodeMLP(torch.cat((x, aggr), dim=-1))
        # print(node_out.shape, edge_out.shape, aggr.shape)
        edge_out = edge_feature + edge_out
        node_out = x + node_out
        return (node_out, edge_out) if self.update_edges else node_out
    ...
    def aggregate(self, inputs, index, ptr=None, dim_size=None):
        out = super().aggregate(inputs, index, ptr, self.num_rec)
        return (inputs, out)
        ...
        # torch.Size([1, 12716, 128]) torch.Size([1, 8409, 128])
        return (inputs, out)


class InteractionNetNew(pyg.nn.MessagePassing):
    def __init__(
        self,
        input_dim,
        hidden_layers=1,
    ...
    def aggregate(self, inputs, index, ptr=None, dim_size=None):
        print(index.shape)
        out = super().aggregate(inputs, index, ptr, inputs.shape[1])
        return (inputs, out)
